...
struct Api_PendingRequest : Codable {
    let result : [Res_PendingRequest]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_PendingRequest].self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_PendingRequest : Codable {
    let id : String?
    let user_id : String?
